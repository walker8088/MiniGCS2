...
class MainWindow(QMainWindow) :
    ...
    def update_gps_raw_int(self, fix_type, lat, lon, alt, eph, satellites_visible):
	self.update_gps_raw_int_signal.emit(fix_type, lat, lon, alt, eph, satellites_visible)
    ...
    def on_update_gps_raw_int(self, fix_type, lat, lon, alt, eph, satellites_visible): 
        gps_types = {0:u'无GPS',1:u'GPS通讯正常', 2:u'GPS啥状态？',3:u'GPS锁定'}
	float_ratio = 10000000
	
	if fix_type < 2:
		msg = gps_types[fix_type]
	else:
		msg = u'%s 经度：%f 纬度：%f 高度：%f 精度:%.2f米 卫星数：%d' \
	...
	self.calibrationAct.setText(u"下一步")
        
	if 'level' in text : 
		info = u"请将机身水平放置后，点击“下一步”按钮"
	elif 'BACK' in text : 
		info = u"请将机身水平翻转放置后，点击“下一步”按钮"
	elif 'LEFT' in text : 
		info = u"请将机身左侧朝下放置后，点击“下一步”按钮"
	elif 'RIGHT' in text : 
		info = u"请将机身右侧朝下放置后，点击“下一步”按钮"
	elif 'UP' in text : 
		info = u"请将机头朝上放置后，点击“下一步”按钮"
	elif 'DOWN' in text : 
		info = u"请将机头朝下放置后，点击“下一步”按钮"
	else:
		info = text
    ...
    def onConnect(self):
        ...
        if (not self.conn_mgr) or (not self.conn_mgr.running) :
            try:
                ...
                self.conn_mgr.open_comm(str(self.portView.currentText()), int(self.baudView.currentText()))
            except :
                self.conn_mgr = None
                return
    ...
    def onConnectUdp(self):
		...
		#self.conn_mgr.open_udp('192.168.43.1', 14550)
                ...
                self.conn_mgr = None
                return
            self.connectUdpAct.setText(u"断开连接")
            self.connectAct.setEnabled(False)
            self.paramsMgrAct.setEnabled(True)
            self.portDetectAct.setEnabled(False)
	    self.calibrationAct.setEnabled(True)
            ...
            self.connectUdpAct.setText(u"连接奥丁")
            self.conn_mgr.close()
            self.conn_mgr = None
    ...
    def onBurnRom(self):
        ...
        QMessageBox.information(self,u'操作提示',u'把未安装存储卡的飞控从USB口接入本台电脑，然后点击“OK”按钮', 
					QMessageBox.Ok)
	...
	if error_count == 0:
		self.info(u"参数文件%s写入成功" % (fileName, ) )
	else :			
		self.info(u"参数文件%s中%d个参数写入失败，请检查硬件。" % (fileName, error_count) )
	...
	if self.accel_cal_count == 6 :
	    self.accel_cal_count == -1
	    self.calibrationAct.setText(u"姿态校准")
    ...
    def createActions(self):
        ...
        self.connectUdpAct =  QAction(u"连接奥丁", self,  triggered=self.onConnectUdp)
	...
	self.portView.currentIndexChanged['QString'].connect(self.onPortViewIndexChanged)
