...
class ParamManager:
    ...
    def handle_mavlink_packet(self, m, master):
        ...
        if m.get_type() == 'PARAM_VALUE':
            ...
            self.observer.show_param(m.param_count, m.param_index, m.param_id,
                                     int(m.param_type),
                                     str(self.mav_param[str(param_id)]))
    ...
    def fetch_check(self, master):
        ...
        if self.param_period.trigger():
            ...
            elif self.mav_param_count != 0 and len(
                    self.mav_param_set) != self.mav_param_count:
                if master.time_since('PARAM_VALUE') >= 1:
                    diff = set(range(self.mav_param_count)).difference(
                        self.mav_param_set)
                    count = 0
