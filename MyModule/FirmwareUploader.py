...
#               return True
        ...
        def identify(self,  observer):
                ...
                if (self.bl_rev < uploader.BL_REV_MIN) or (self.bl_rev > uploader.BL_REV_MAX):
                        observer.info("Unsupported bootloader protocol %d" % uploader.INFO_BL_REV)
        ...
        def upload(self, fw,  observer):
                ...
                observer.info(u"正在擦除...")
                self.__erase()

                observer.info(u"正在烧录...")
                self.__program(fw)

                observer.info(u"正在校验...")
                if self.bl_rev == 2:
                        self.__verify_v2(fw)
                else:
                        self.__verify_v3(fw)

                observer.info(u"校验完成，正在重启飞控...")
                self.__reboot()
                self.port.close()
...
def do_upload(frimware_file,  port,  observer):
    # Load the firmware file
    fw = firmware(frimware_file)
    observer.info("Loaded firmware for %x,%x, waiting for the bootloader..." % (fw.property('board_id'), fw.property('board_revision')))

    observer.info("Trying %s" % port)

    # create an uploader attached to the port
    up = uploader(port, 57600)
    
    # identify the bootloader
    up.identify(observer)
    print("Found board %x,%x bootloader rev %x on %s" % (up.board_type, up.board_rev, up.bl_rev, port))

    try:
            # ok, we have a bootloader, try flashing it
            up.upload(fw, observer)
    except RuntimeError as ex:
            # print the error
            observer.info("ERROR: %s" % ex.args)
