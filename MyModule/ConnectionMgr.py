...
class ConnectionManager():
	...
	self.conn.mav.command_long_send(self.status.target_system, self.status.target_component,
    ...
    def on_msg_heartbeat(self, m, master):
        ...
        if armed != self.status.armed:
            ...
            if armed:
                self.observer.info(u"已解锁")
            else:
                self.observer.info(u"已锁定")
    ...
    def on_msg_gps_raw_int(self, m, master):
        self.observer.update_gps_raw_int(m.fix_type, m.lat, m.lon, m.alt, m.eph, m.satellites_visible) 
        """
        fix_type : 0, 
        lat : 0, 
        lon : 0, 
    ...
    def on_recv_callback(self, m, master):
	    ...
	    msgs = []
            if raw_msgs[0].startswith('time_usec') or raw_msgs[0].startswith('time_boot_ms'):
		index = 1
	    else:
		index = 0
	...
	self.baud = baud 
        ...
        self.conn = mavutil.mavlink_connection(self.com_port, autoreconnect=True, baud=self.baud)
