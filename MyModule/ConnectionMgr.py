...
def report_altitude(altitude):
    ...
    master = self.master()
    if getattr(self.console, 'ElevationMap',
               None) is not None and self.settings.basealt != 0:
    ...
    if (int(self.settings.altreadout) > 0
            and math.fabs(self.status.altitude -
                          self.status.last_altitude_announce) >= int(
                              self.settings.altreadout)):
        self.status.last_altitude_announce = self.status.altitude
        rounded_alt = int(self.settings.altreadout) * (
            (self.settings.altreadout / 2 + int(self.status.altitude)) /
...
class ConnectionManager():
    ...
    def cmd_accel_cal(self):
        '''do a full 3D accel calibration'''
        self.conn.mav.command_long_send(
            self.status.target_system, self.status.target_component,
            mavutil.mavlink.MAV_CMD_PREFLIGHT_CALIBRATION, 0, 0, 0, 0, 0, 1, 0,
            0)

        self.accelcal_count = 0
        self.accelcal_wait_enter = False

    def accel_cal_ready(self, count):
        self.conn.mav.command_ack_send(count, 1)
        self.accelcal_wait_enter = False

    def cmd_gyro_cal(self):
        '''do a full gyro calibration'''
        self.conn.mav.command_long_send(
            self.status.target_system, self.status.target_component,
            mavutil.mavlink.MAV_CMD_PREFLIGHT_CALIBRATION, 0, 1, 0, 0, 0, 0, 0,
            0)

    def cmd_mag_cal(self, cmd):
    ...
    def start_request_data_stream(self, master):
        master.mav.request_data_stream_send(self.status.target_system,
                                            self.status.target_component, 0,
                                            10, 1)
    ...
    def on_msg_heartbeat(self, m, master):
        ...
        if armed != self.status.armed:
            ...
            if armed:
                self.observer.info("已解锁")
            else:
                self.observer.info("已锁定")
    ...
    def on_msg_statustext(self, m, master):
        if self.accelcal_count != -1:
            ...
            if text.startswith('Place '):
                ...
                self.accelcal_count += 1
                self.observer.update_accel_cal(text, self.accelcal_count)
                if self.accelcal_count >= 6:
                    self.accelcal_count = -1
        else:
            severitys = ("EMERGENCY", "ALERT", "CRITICAL", "ERROR", "WARNING",
                         "NOTICE", "INFO", "DEBUG")
            self.observer.info(severitys[m.severity] + ": " + m.text)

    def on_msg_rc_channels_raw(self, m, master):
        self.status.raw_rc_chanels = (m.chan1_raw, m.chan2_raw, m.chan3_raw,
                                      m.chan4_raw, m.chan5_raw, m.chan6_raw,
                                      m.chan7_raw, m.chan8_raw)

    def on_msg_servo_output_raw(self, m, master):
        self.status.raw_servo_outs = (m.servo1_raw, m.servo2_raw, m.servo3_raw,
                                      m.servo4_raw, m.servo5_raw, m.servo6_raw,
                                      m.servo7_raw, m.servo8_raw)

    def on_msg_gps_raw_int(self, m, master):
        self.observer.update_gps_raw_int(m.fix_type, m.lat, m.lon, m.alt,
                                         m.eph, m.satellites_visible)
        """
        fix_type : 0, 
        lat : 0, 
        lon : 0, 
    ...
    def on_msg_vfr_hud(self, m, master):
        pass
        """
        airspeed : 0.0, 
        groundspeed : 0.0, 
        ...
        with self.read_lock:
            while retries > 0 and not got_ack:
                ...
                while time.time() - tstart < 1:
                    self.conn.recv_msg()
                    ack = self.conn.recv_match(type='PARAM_VALUE',
                                               blocking=True)
                    if ack == None:
                        time.sleep(0.1)
                        continue
    ...
    def on_recv_callback(self, m, master):
	    ...
	    items = str(m).split()[1:]
	    for it in items :
		if it.startswith('{time_usec') or it.startswith('{time_boot_ms'):
			continue
		show_items.append(it)
	    '''
            raw_msg = str(m)
            index = raw_msg.find("{")
            raw_msgs = raw_msg[index + 1:-1].split(",")  #

            msgs = []
            if raw_msgs[0].startswith('time_usec') or raw_msgs[0].startswith(
                    'time_boot_ms'):
                index = 1
            else:
                index = 0
    ...
    def open_tcp(self, host, port):
        ...
        self.conn = mavutil.mavlink_connection("tcp:%s:%d" % (host, port))
    ...
    def open_udp(self, host, port):
        ...
        self.conn = mavutil.mavlink_connection("udp:0.0.0.0:%d" % port)
        self.conn.last_address = (host, port)

        return self.after_open()

    def open_comm(self, com_port, baud):
        ...
        self.conn = mavutil.mavlink_connection(self.com_port,
                                               autoreconnect=True,
                                               baud=self.baud)
